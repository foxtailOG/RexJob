...
import type { ResumeState, PersonalInfo, TemplateOption, ColorScheme, FontFamily, SectionVisibility, ResumeData } from '../types';
// Dummy import for jsPDF from window
...
const ControlPanel: React.FC<{ state: ResumeState; dispatch: React.Dispatch<Action> }> = ({ state, dispatch }) => {
    ...
    return (
    <div className="p-6 space-y-8 bg-white/60 dark:bg-dark-surface/60 backdrop-blur-xl rounded-2xl shadow-lg shadow-primary-100/50 dark:shadow-black/20 border border-white/50 dark:border-slate-700">
        <div>
            <h3 className="font-bold text-lg mb-4 text-primary-800 dark:text-primary-100">Templates</h3>
            <div className="grid grid-cols-2 gap-4">
                {TemplateOptions.map(template => (
                    <button key={template.id} onClick={() => dispatch({ type: 'UPDATE_STYLE', payload: { field: 'template', value: template.id } })}
                        className={`p-3 text-left border-2 rounded-xl transition-all duration-300 ${state.style.template === template.id ? 'border-transparent ring-2 ring-secondary-500 bg-secondary-500/10' : 'border-slate-200/80 dark:border-slate-600 hover:border-secondary-400'}`}>
                        ...
                        <p className="text-sm font-semibold capitalize mt-2 text-primary-900 dark:text-primary-200">{template.name}</p>
        ...
        <div>
            <h3 className="font-bold text-lg mb-4 text-primary-800 dark:text-primary-100">Styling</h3>
            <div className="space-y-4">
                <div>
                    ...
                    <select value={state.style.fontFamily} onChange={e => dispatch({ type: 'UPDATE_STYLE', payload: { field: 'fontFamily', value: e.target.value }})} 
                        className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-slate-300/80 bg-white/80 dark:bg-slate-700 dark:border-slate-600 dark:text-white focus:outline-none focus:ring-secondary-500 focus:border-secondary-500 sm:text-sm rounded-md">
                        {FontFamilies.map(font => <option key={font}>{font}</option>)}
                    </select>
                </div>
                <div>
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Color Scheme</label>
                    <div className="mt-2 grid grid-cols-3 gap-3">
                        {(Object.keys(ColorSchemes) as ColorScheme[]).map(color => (
                            <button key={color} onClick={() => dispatch({ type: 'UPDATE_STYLE', payload: { field: 'colorScheme', value: color } })}
                                className={`flex items-center space-x-2 p-2 rounded-lg border-2 transition-all ${state.style.colorScheme === color ? 'border-transparent ring-2 ring-secondary-500' : 'border-slate-200/80 dark:border-slate-600 hover:border-secondary-400'}`}>
                                ...
                                <span className="text-sm text-slate-700 dark:text-slate-300">{ColorSchemes[color].name}</span>
                            </button>
                        ))}
                    </div>
                </div>
         ...
         <div>
            <h3 className="font-bold text-lg mb-4 text-primary-800 dark:text-primary-100">Layout</h3>
            <div className="space-y-4">
                 <div>
                    ...
                    <input type="range" min="8" max="14" step="1" value={state.style.fontSize}
                        ...
                        className="w-full h-2 bg-slate-200 dark:bg-slate-600 rounded-lg appearance-none cursor-pointer mt-1 accent-secondary-500" />
                </div>
                 <div>
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300 flex justify-between">
        ...
        <div>
            <h3 className="font-bold text-lg mb-4 text-primary-800 dark:text-primary-100">Sections</h3>
            <div className="space-y-3 bg-slate-500/5 dark:bg-slate-900/20 p-4 rounded-lg" onDrop={handleDrop} onDragOver={e => e.preventDefault()}>
                {state.sectionOrder.map((section, index) => (
                    <div 
                        key={section} 
                        className="flex items-center justify-between bg-white/80 dark:bg-slate-700/50 p-2 rounded-md draggable-section transition-all"
                        draggable
                    ...
                    >
                        <div className="flex items-center">
                            <DragHandleIcon />
                            <span className="text-slate-700 dark:text-slate-200 capitalize text-sm font-medium ml-2">{section.replace(/([A-Z])/g, ' $1').trim()}</span>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                            ...
                            <div className="w-11 h-6 bg-slate-200 dark:bg-slate-600 rounded-full peer peer-focus:ring-2 peer-focus:ring-secondary-400 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-secondary-500"></div>
                        </label>
                    </div>
                ))}
            </div>
...
const FormPanel: React.FC<{ state: ResumeState; dispatch: React.Dispatch<Action> }> = ({ state, dispatch }) => {
    ...
    const formInputClass = "w-full p-2 border border-slate-300/80 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-secondary-500 focus:border-transparent transition-shadow bg-white/50 dark:bg-slate-800/50 placeholder:text-slate-400 dark:placeholder:text-slate-500 dark:text-white";
    ...
    const cardClass = "p-6 bg-white/60 dark:bg-dark-surface/60 backdrop-blur-xl rounded-2xl shadow-lg shadow-primary-100/50 dark:shadow-black/20 border border-white/50 dark:border-slate-700";

    const EmptyState: React.FC<{icon: React.ReactNode, title: string, text: string}> = ({icon, title, text}) => (
        <div className="text-center text-slate-500 dark:text-slate-400 py-6 bg-slate-500/5 dark:bg-slate-900/20 rounded-lg border border-dashed border-slate-300/70 dark:border-slate-700">
            {icon}
            <p className="mt-2 font-semibold text-slate-600 dark:text-slate-300">{title}</p>
    ...
    <div className="space-y-6">
        <div className={cardClass}>
            <h3 className="font-bold text-lg mb-4 text-primary-800 dark:text-primary-100">Personal Information</h3>
        ...
        {sections.professionalSummary && <div className={cardClass}>
            <h3 className="font-bold text-lg mb-4 text-primary-800 dark:text-primary-100">Professional Summary</h3>
             ...
             <p className="text-xs text-right text-slate-500 dark:text-slate-400 mt-1">{data.professionalSummary.length}/300 characters</p>
             <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 p-2 bg-slate-500/10 dark:bg-slate-900/20 rounded-md">Tip: Keep it concise and impactful (2-3 sentences).</p>
        </div>}

        {sections.workExperience && <div className={cardClass}>
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-primary-800 dark:text-primary-100">Work Experience</h3>
            ...
            {data.workExperience.map((exp, index) => (
                <div key={exp.id} className="p-4 border border-slate-200/80 dark:border-slate-700 rounded-lg space-y-3 bg-white/50 dark:bg-slate-800/30">
                    <div className="flex justify-between items-center">
                        <p className="font-semibold text-slate-700 dark:text-slate-200">Experience #{index + 1}</p>
        ...
        {sections.education && <div className={cardClass}>
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-primary-800 dark:text-primary-100">Education</h3>
            ...
            {data.education.map((edu, index) => (
                <div key={edu.id} className="p-4 border border-slate-200/80 dark:border-slate-700 rounded-lg space-y-3 bg-white/50 dark:bg-slate-800/30">
                    <div className="flex justify-between items-center">
                        <p className="font-semibold text-slate-700 dark:text-slate-200">Education #{index + 1}</p>
        ...
        {sections.projects && <div className={cardClass}>
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-primary-800 dark:text-primary-100">Projects</h3>
                <button onClick={() => dispatch({type: 'ADD_ITEM', payload: {section: 'projects'}})} className={addButtonClass}><AddIcon /> Add Project</button>
            </div>
            <div className="space-y-4">
            {data.projects.map((proj, index) => (
                <div key={proj.id} className="p-4 border border-slate-200/80 dark:border-slate-700 rounded-lg space-y-3 bg-white/50 dark:bg-slate-800/30">
                    <div className="flex justify-between items-center">
                        <p className="font-semibold text-slate-700 dark:text-slate-200">Project #{index + 1}</p>
        ...
        {sections.certifications && <div className={cardClass}>
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-primary-800 dark:text-primary-100">Certifications</h3>
            ...
            {data.certifications.map((cert, index) => (
                <div key={cert.id} className="p-4 border border-slate-200/80 dark:border-slate-700 rounded-lg space-y-3 bg-white/50 dark:bg-slate-800/30">
                    <div className="flex justify-between items-center">
                        <p className="font-semibold text-slate-700 dark:text-slate-200">Certification #{index + 1}</p>
         ...
         {sections.skills && <div className={cardClass}>
             <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-primary-800 dark:text-primary-100">Skills</h3>
             ...
             {data.skills.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                    {data.skills.map((skill) => (
                         <div key={skill.id} className="flex items-center bg-slate-500/10 dark:bg-slate-700/50 rounded-full">
                            <input value={skill.name} onChange={e => dispatch({type: 'UPDATE_ITEM', payload: {section: 'skills', id: skill.id, field: 'name', value: e.target.value}})} placeholder="Skill" className="bg-transparent px-4 py-1.5 text-sm w-36 focus:outline-none dark:text-white" />
...
const ResumeTemplate: React.FC<{ state: ResumeState }> = ({ state }) => {
    ...
    if (isTwoColumn) {
        ...
        return (
             <div className={`${fontClass} bg-white p-10 text-slate-800`}>
    ...
    const personalDetails = [data.personalInfo.email, data.personalInfo.phone, data.personalInfo.location, data.personalInfo.linkedin, data.personalInfo.website].filter(Boolean);
    return (
        <div className={`${fontClass} bg-white p-10 text-slate-800`} style={{ display: 'flex', flexDirection: 'column', gap: `${style.rowGap}rem` }}>
...
const ResumePreview: React.FC<{ state: ResumeState; resumeRef: React.RefObject<HTMLDivElement> }> = ({ state, resumeRef }) => {
    ...
    return (
        <div className="w-full h-full bg-slate-200/50 dark:bg-slate-900/50 p-4 md:p-8 overflow-y-auto">
            ...
            <div ref={resumeRef} className="flex flex-col items-center space-y-8">
                {pages.length > 0 ? pages.map((page, pageIndex) => (
                    ...
                    >
                     ...
                     <div className={`aspect-[8.5/11] w-[210mm] shadow-2xl shadow-slate-400/50 bg-white ${fontClass} overflow-hidden flex items-center justify-center text-slate-800`}>
...
const ResumeBuilder: React.FC<ResumeBuilderProps> = ({ onBackToHome }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        const primaryColor = colorMap[state.style.colorScheme];
        ...
        const drawText = (text: string, x: number, fontSize: number, isBold: boolean = false, color: string = '#000000', maxWidthOverride?: number) => {
            pdf.setFont('helvetica', isBold ? 'bold' : 'normal');
            pdf.setFontSize(fontSize);
            pdf.setTextColor(color);
            const lines = pdf.splitTextToSize(text, maxWidthOverride || maxWidth);
        ...
        if (isTwoColumn) {
            const sideColumnWidth = (pageWidth * state.style.columnWidth / 100) - margin;
            const mainColumnWidth = pageWidth - sideColumnWidth - (margin * 3);
            const sideColumnX = pageWidth - margin - sideColumnWidth;

            pdf.setFont('helvetica', 'bold');
            pdf.setFontSize(24);
            pdf.setTextColor(primaryColor);
            pdf.text(state.data.personalInfo.fullName || 'Your Name', margin, yPos);
            yPos += 30;

            let mainYPos = yPos;
            let sideYPos = yPos;

            const renderMainSection = (key: keyof SectionVisibility) => {
                yPos = mainYPos;

                switch (key) {
                    case 'professionalSummary':
                        if (!state.data.professionalSummary) break;
                        drawSectionHeader('SUMMARY');
                        drawText(state.data.professionalSummary, margin, state.style.fontSize, false, '#000000', mainColumnWidth);
                        yPos += 10;
                        break;

                    case 'workExperience':
                        if (state.data.workExperience.length === 0) break;
                        drawSectionHeader('WORK EXPERIENCE');
                        state.data.workExperience.forEach(exp => {
                            checkPageBreak(40);
                            drawText(exp.jobTitle || 'Job Title', margin, state.style.fontSize + 1, true, '#000000', mainColumnWidth);
                            drawText(exp.company || 'Company', margin, state.style.fontSize, false, '#4b5563', mainColumnWidth);
                            drawText(`${exp.startDate} - ${exp.endDate}`, margin, state.style.fontSize - 1, false, '#6b7280', mainColumnWidth);
                            yPos += 5;
                            if (exp.description) {
                                ...
                                descLines.forEach(line => {
                                    ...
                                    if (cleanLine) {
                                        ...
                                        pdf.setFontSize(state.style.fontSize);
                                        pdf.setTextColor('#374151');
                                        pdf.text('•', margin, yPos);
                                        const wrapped = pdf.splitTextToSize(cleanLine, mainColumnWidth - 15);
                                        wrapped.forEach((wrappedLine: string) => {
                                            pdf.text(wrappedLine, margin + 15, yPos);
                                            yPos += (state.style.fontSize * 1.2);
                                        });
                                    }
                                });
                            }
                    ...
                    case 'education':
                        if (state.data.education.length === 0) break;
                        drawSectionHeader('EDUCATION');
                        state.data.education.forEach(edu => {
                            checkPageBreak(30);
                            drawText(edu.degree || 'Degree', margin, state.style.fontSize + 1, true, '#000000', mainColumnWidth);
                            drawText(edu.institution || 'Institution', margin, state.style.fontSize, false, '#4b5563', mainColumnWidth);
                            drawText(`${edu.startDate} - ${edu.endDate}`, margin, state.style.fontSize - 1, false, '#6b7280', mainColumnWidth);
                            yPos += 10;
                        });
                        break;

                    case 'projects':
                        if (state.data.projects.length === 0) break;
                        drawSectionHeader('PROJECTS');
                        state.data.projects.forEach(proj => {
                            checkPageBreak(40);
                            drawText(proj.projectName || 'Project Name', margin, state.style.fontSize + 1, true, '#000000', mainColumnWidth);
                            if (proj.technologies) {
                                drawText(`Technologies: ${proj.technologies}`, margin, state.style.fontSize - 1, false, '#4b5563', mainColumnWidth);
                            }
                            if (proj.description) {
                                drawText(proj.description, margin, state.style.fontSize, false, '#374151', mainColumnWidth);
                            }
                            yPos += 10;
                        });
                        break;
            ...
            const renderSideSection = (key: keyof SectionVisibility) => {
                yPos = sideYPos;

                const sectionHeaderSide = (title: string) => {
                    checkPageBreak(20);
                    pdf.setFont('helvetica', 'bold');
                    pdf.setFontSize(12);
                    pdf.setTextColor(primaryColor);
                    pdf.text(title.toUpperCase(), sideColumnX, yPos);
                    yPos += 3;
                    pdf.setDrawColor(primaryColor);
                    pdf.setLineWidth(1.5);
                    pdf.line(sideColumnX, yPos, pageWidth - margin, yPos);
                    yPos += 12;
                };

                if (key === 'skills') {
                    if (state.data.skills.length === 0) return;
                    sectionHeaderSide('SKILLS');
                    const skillsText = state.data.skills.map(s => s.name).filter(Boolean).join(' • ');
                    if (skillsText) {
                        const skillLines = pdf.splitTextToSize(skillsText, sideColumnWidth);
                        pdf.setFont('helvetica', 'normal');
                        pdf.setFontSize(state.style.fontSize);
                        pdf.setTextColor('#374151');
                        skillLines.forEach((line: string) => {
                            checkPageBreak(15);
                            pdf.text(line, sideColumnX, yPos);
                            yPos += (state.style.fontSize * 1.2);
                        });
                    }
                    yPos += 10;
                } else if (key === 'certifications') {
                    if (state.data.certifications.length === 0) return;
                    sectionHeaderSide('CERTIFICATIONS');
                    state.data.certifications.forEach(cert => {
                        checkPageBreak(30);
                        pdf.setFont('helvetica', 'bold');
                        pdf.setFontSize(state.style.fontSize);
                        pdf.setTextColor('#000000');
                        const certLines = pdf.splitTextToSize(cert.name || 'Certification', sideColumnWidth);
                        ...
                        pdf.setFont('helvetica', 'normal');
                        pdf.setFontSize(state.style.fontSize - 1);
                        pdf.setTextColor('#4b5563');
                        const orgLines = pdf.splitTextToSize(cert.organization || '', sideColumnWidth);
                        orgLines.forEach((line: string) => {
                            pdf.text(line, sideColumnX, yPos);
                            yPos += ((state.style.fontSize - 1) * 1.2);
                        });
                        yPos += 8;
                    });
                }
            ...
            pdf.setFontSize(12);
            pdf.setTextColor(primaryColor);
            pdf.text('CONTACT', sideColumnX, yPos);
            yPos += 3;
            pdf.setDrawColor(primaryColor);
            pdf.setLineWidth(1.5);
            pdf.line(sideColumnX, yPos, pageWidth - margin, yPos);
            ...
            const personalDetails = [
                state.data.personalInfo.email,
                state.data.personalInfo.phone,
                state.data.personalInfo.location,
                state.data.personalInfo.linkedin,
                state.data.personalInfo.website
            ].filter(Boolean);
            ...
            personalDetails.forEach(detail => {
                checkPageBreak(15);
                const lines = pdf.splitTextToSize(detail, sideColumnWidth);
            ...
            state.sectionOrder.forEach(sectionKey => {
                if (!state.sections[sectionKey]) return;
                if (mainSections.includes(sectionKey)) {
                    renderMainSection(sectionKey);
                } else if (sideSections.includes(sectionKey)) {
                    renderSideSection(sectionKey);
                }
            });
            ...
            pdf.setFontSize(22);
            pdf.setTextColor(primaryColor);
            const nameWidth = pdf.getTextWidth(state.data.personalInfo.fullName || 'Your Name');
            pdf.text(state.data.personalInfo.fullName || 'Your Name', (pageWidth - nameWidth) / 2, yPos);
            yPos += 15;
            ...
            if (personalDetails.length > 0) {
                ...
                pdf.setFontSize(state.style.fontSize - 1);
                pdf.setTextColor('#4b5563');
                const contactLine = personalDetails.join(' • ');
            ...
            state.sectionOrder.forEach(sectionKey => {
                ...
                switch (sectionKey) {
                    case 'professionalSummary':
                        if (!state.data.professionalSummary) break;
                        drawSectionHeader('SUMMARY');
                        drawText(state.data.professionalSummary, margin, state.style.fontSize);
                        yPos += 10;
                        break;
                    ...
                    case 'workExperience':
                        ...
                        state.data.workExperience.forEach(exp => {
                            ...
                            pdf.setFontSize(state.style.fontSize + 1);
                            pdf.setTextColor('#000000');
                            pdf.text(exp.jobTitle || 'Job Title', margin, yPos);

                            pdf.setFont('helvetica', 'normal');
                            pdf.setFontSize(state.style.fontSize - 1);
                            pdf.setTextColor('#6b7280');
                            pdf.text(`${exp.startDate} - ${exp.endDate}`, pageWidth - margin, yPos, { align: 'right' });
                            yPos += (state.style.fontSize + 1) * 1.2;

                            drawText(exp.company || 'Company', margin, state.style.fontSize, false, '#4b5563');
                            yPos += 5;
                            ...
                            if (exp.description) {
                                ...
                                descLines.forEach(line => {
                                    ...
                                    if (cleanLine) {
                                        ...
                                        pdf.setTextColor('#374151');
                                        pdf.text('•', margin, yPos);
                                        const wrapped = pdf.splitTextToSize(cleanLine, maxWidth - 15);
                                        wrapped.forEach((wrappedLine: string) => {
                                            pdf.text(wrappedLine, margin + 15, yPos);
                                            yPos += (state.style.fontSize * 1.2);
                                        });
                    ...
                    case 'education':
                        ...
                        state.data.education.forEach(edu => {
                            ...
                            pdf.setFontSize(state.style.fontSize - 1);
                            pdf.setTextColor('#6b7280');
                            pdf.text(`${edu.startDate} - ${edu.endDate}`, pageWidth - margin, yPos, { align: 'right' });
                            yPos += (state.style.fontSize + 1) * 1.2;

                            drawText(edu.institution || 'Institution', margin, state.style.fontSize, false, '#4b5563');
                            yPos += 10;
                        });
                        break;
                    ...
                    case 'projects':
                        ...
                        state.data.projects.forEach(proj => {
                            checkPageBreak(40);
                            drawText(proj.projectName || 'Project Name', margin, state.style.fontSize + 1, true);
                            if (proj.technologies) {
                                drawText(`Technologies: ${proj.technologies}`, margin, state.style.fontSize - 1, false, '#4b5563');
                            }
                            if (proj.description) {
                                drawText(proj.description, margin, state.style.fontSize, false, '#374151');
                            }
                            yPos += 10;
                        });
                        break;
                    ...
                    case 'skills':
                        if (state.data.skills.length === 0) break;
                        drawSectionHeader('SKILLS');
                        const skillsText = state.data.skills.map(s => s.name).filter(Boolean).join(' • ');
                        if (skillsText) {
                            drawText(skillsText, margin, state.style.fontSize, false, '#374151');
                        }
                        yPos += 10;
                        break;

                    case 'certifications':
                        if (state.data.certifications.length === 0) break;
                        ...
                        state.data.certifications.forEach(cert => {
                            ...
                            drawText(cert.organization || 'Organization', margin, state.style.fontSize, false, '#4b5563');
                            if (cert.about) {
                                drawText(cert.about, margin, state.style.fontSize - 1, false, '#6b7280');
                            }
                            yPos += 10;
                        });
                        break;
    ...
    return (
         <div className="animated-gradient-bg min-h-screen flex flex-col font-sans text-slate-800 dark:text-slate-200">
            <header className="bg-white/70 dark:bg-dark-surface/70 backdrop-blur-lg sticky top-0 z-30 shadow-sm dark:shadow-black/20 flex-shrink-0">
                <div className="max-w-[100rem] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
                     <button onClick={onBackToHome} className="text-primary-600 hover:text-primary-700 dark:text-primary-300 dark:hover:text-primary-200 font-semibold flex items-center group">
                    ...
                    <h1 className="text-xl font-bold text-primary-800 dark:text-primary-100">Resume Builder</h1>
